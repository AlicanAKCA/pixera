...
def load_images(size=(64,64)):
  imagesFN =  os.listdir(f"{os.getcwd()}/dataset/train/panda/B") 
  images_Path = f"{os.getcwd()}/dataset/train/panda/B"
  for i in imagesFN:
    try:
...
class GAN():
    ...
    def train(self, epochs, batch_size=128, metrics_update=50, save_images=100, save_model=2000):
        ...
        for epoch in range(epochs):
            ...
            if epoch % save_model == 0:
                self.generator.save(f"{os.getcwd()}/results/generators/generator_%d" % epoch)
                self.discriminator.save(f"{os.getcwd()}/results/discriminators/discriminator_%d" % epoch)
    ...
    def save_images(self, epoch):
        ...
        fig.savefig(f"{os.getcwd()}/results/pandaS_%d.png" % epoch)
        plt.close()

gan=GAN()
gan.train(epochs=15000, batch_size=256, metrics_update=200, save_images=150, save_model=15000)